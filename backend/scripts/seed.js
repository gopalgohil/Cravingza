...
const restaurantsData = [
  ...
  {
    _id: new mongoose.Types.ObjectId("6a71cf8dab29fa8868772237"),
    name: "Burger Boss",
...
async function seed() {
  try {
    ...
    for (const rData of restaurantsData) {
      ...
      let restaurant = await Restaurant.findOne({ name: restDetails.name });
      if (!restaurant) {
        restaurant = new Restaurant(restDetails);
        await restaurant.save();
        console.log(`Seeded Restaurant: ${restaurant.name}${restDetails.owner ? " (Owner Linked)" : ""}`);
      } else {
        if (restDetails.name === "Burger Boss" && targetOwner) {
          restaurant.owner = targetOwner._id;
        }
      ...
      for (const mItem of menu) {
        let menuItem = await MenuItem.findOne({ restaurant: restaurant._id, name: mItem.name });
        if (!menuItem) {
      ...
      console.log(`  Seeded/Verified ${menu.length} menu items for ${restaurant.name}.`);
    }

    console.log("Seeding complete! All rich restaurants restored.");
    process.exit(0);
  } catch (error) {
